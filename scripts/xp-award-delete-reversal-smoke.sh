...
cleanup() {
  ...
  if [[ -n "${DATABASE_URL:-}" ]]; then
    psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -v planet_slug="xp-delete-reversal-$STAMP" >/dev/null 2>&1 <<'SQL' || true
...
echo "[$SCRIPT_NAME] Seeding planet and XP reward tier..."
planet_id="$(
  psql "$DATABASE_URL" -q -tA -v ON_ERROR_STOP=1 \
    -v planet_slug="$planet_slug" \
    -v planet_name="XP Delete Reversal Planet $STAMP" <<'SQL'
INSERT INTO planets (slug, name, completion_percent)
VALUES (:'planet_slug', :'planet_name', 40.000)
RETURNING id;
SQL
)"
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -v match_name="$match_name" -v planet_id="$planet_id" <<'SQL'
INSERT INTO xp_reward_tiers (mission_name_match, xp_amount, planet_id, planet_progress_percent)
VALUES (:'match_name', 30, :'planet_id', 7.125);
SQL
...
curl -fsS -X POST "$BASE_URL/v1/operations/$operation_id/finish" \
  ...
  assert_json "data.ok === true && data.xp_award?.awarded === true && data.xp_award.award_status === 'awarded' && data.xp_award.xp_amount === 30 && data.xp_award.players_awarded === 2 && data.planet_progress_award?.awarded === true && data.planet_progress_award.award_status === 'awarded' && data.planet_progress_award.completion_percent_after === '47.125'"
