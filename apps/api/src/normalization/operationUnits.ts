...
export async function syncOperationUnitsForParticipants(
  ...
  const playerUnits = await tx.query(
    ...
    fallback_memberships AS (
      SELECT DISTINCT ON (pp.operation_player_uid)
        pp.operation_player_uid,
        cup.unit_id
      FROM participant_players pp
      ...
      ORDER BY pp.operation_player_uid, up.assignment_priority DESC, up.updated_at DESC, cup.unit_id
    ),
    selected_player_units AS (
      ...
      LEFT JOIN valid_preferences vp ON vp.operation_player_uid = pp.operation_player_uid
      LEFT JOIN fallback_memberships fm ON fm.operation_player_uid = pp.operation_player_uid
    )
